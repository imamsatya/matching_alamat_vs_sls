...
CONFIG = {
    ...
    "TABLE_NAME": "matcha_datatemp_direktori_sekolah_20260520",

    "COL_IDSBR": "idsbr_algoritma_bm25v12",
    "COL_PERUSAHAAN_ID": "perusahaan_id_algoritma_bm25v12",
    "COL_POSSIBLE_MATCH": "possible_match_algoritma_bm25v12",
    ...
    "BULK_SIZE": 500,
    "LOG_FILE": "matching_bm25_v12_multi.log",
    "CHECKPOINT_FILE": "matching_bm25_v12_checkpoint.json",
...
# %% Helper Functions
# Standarisasi nama sekolah — urutan dari panjang ke pendek agar tidak salah substitusi
SCHOOL_STANDARDIZATION = [
    (r'\bnegri\b', 'negeri'),
    (r'\bsekolah dasar negeri\b', 'sd negeri'),
    (r'\bsekolah dasar\b', 'sd'),
    (r'\bsekolah menengah pertama negeri\b', 'smp negeri'),
    (r'\bsekolah menengah pertama\b', 'smp'),
    (r'\bsekolah menengah atas negeri\b', 'sma negeri'),
    (r'\bsekolah menengah atas\b', 'sma'),
    (r'\bsekolah menengah kejuruan negeri\b', 'smk negeri'),
    (r'\bsekolah menengah kejuruan\b', 'smk'),
    (r'\bsekolah luar biasa negeri\b', 'slb negeri'),
    (r'\bsekolah luar biasa\b', 'slb'),
    (r'\bmadrasah ibtidaiyah negeri\b', 'mi negeri'),
    (r'\bmadrasah ibtidaiyah\b', 'mi'),
    (r'\bmadrasah tsanawiyah negeri\b', 'mts negeri'),
    (r'\bmadrasah tsanawiyah\b', 'mts'),
    (r'\bmadrasah aliyah negeri\b', 'ma negeri'),
    (r'\bmadrasah aliyah\b', 'ma'),
    (r'\btaman kanak kanak negeri\b', 'tk negeri'),
    (r'\btaman kanak kanak\b', 'tk'),
    (r'\braudhatul athfal\b', 'ra'),
    (r'\bkelompok bermain\b', 'kb'),
    (r'\bpendidikan anak usia dini\b', 'paud'),
    
    (r'\bsdn\b', 'sd negeri'),
    (r'\bsmpn\b', 'smp negeri'),
    (r'\bsman\b', 'sma negeri'),
    (r'\bsmkn\b', 'smk negeri'),
    (r'\bslbn\b', 'slb negeri'),
    (r'\bmin\b', 'mi negeri'),
    (r'\bmtsn\b', 'mts negeri'),
    
    (r'\bsd n\b', 'sd negeri'),
    (r'\bsmp n\b', 'smp negeri'),
    (r'\bsma n\b', 'sma negeri'),
    (r'\bsmk n\b', 'smk negeri'),
    (r'\bslb n\b', 'slb negeri'),
    (r'\bmi n\b', 'mi negeri'),
    (r'\bmts n\b', 'mts negeri'),
    (r'\bma n\b', 'ma negeri'),
    (r'\btkn\b', 'tk negeri'),
    (r'\btk n\b', 'tk negeri'),
    (r'\bupt spf\b', ''),
    (r'\bupt\b', ''),
    (r'\bpk susu\b', 'pangkalan susu'),
    
    (r'\bsps\b', 'satuan paud sejenis'),
    (r'\bsmks\b', 'smk swasta'),
    (r'\bsds\b', 'sd swasta'),
    (r'\bsmps\b', 'smp swasta'),
    (r'\bsmas\b', 'sma swasta'),
    (r'\bmis\b', 'mi swasta'),
    (r'\bmtss\b', 'mts swasta'),
    (r'\btks\b', 'tk swasta'),
    (r'\b(tk|kb|sd|smp|sma|smk)it\b', r'\1 islam terpadu'),
]
SCHOOL_STANDARDIZATION_COMPILED = [(re.compile(p), r) for p, r in SCHOOL_STANDARDIZATION]

# Normalisasi angka — hanya dipakai di compare_tokens(), BUKAN di normalize()
# Agar tidak salah normalisasi "satu atap", "lima benua", dll.
ROMAN_TO_INT = {
...
def normalize_number_token(token):
    """Normalize number representations to digit string.
    Returns normalized digit string, or None if token is not a number."""
    if token.isdigit():
        return str(int(token))  # strip leading zeros: '01' → '1'
    if token in ROMAN_TO_INT:
        return ROMAN_TO_INT[token]
    if token in NUMBER_WORDS_TO_INT:
        return NUMBER_WORDS_TO_INT[token]
    return None

def extract_base_name(name_raw):
    """Remove content inside <> and () from SBR name before normalize.
    'TK An-Nisaa <Sultan Mursyidin>' → 'TK An-Nisaa'
    'SD IT AZ-ZAHRA (Terpadu)' → 'SD IT AZ-ZAHRA'
    """
    if not name_raw:
        return str(name_raw)
    name = str(name_raw)
...
def single_char_match(name1, name2):
    """Check if single characters (like A, B, C in 'TK II A') conflict."""
    t1 = set(name1.split())
    t2 = set(name2.split())
    chars1 = {w for w in t1 if len(w) == 1 and w.isalpha()}
    chars2 = {w for w in t2 if len(w) == 1 and w.isalpha()}
    if chars1 and chars2 and not chars1.intersection(chars2):
        return False
    return True

def numbers_match(name1, name2):
    """Check if all numbers in both normalized names match (after normalization).
    Prevents false match: 'sd negeri 16 simeulue barat' vs 'sd negeri 6 simeulue barat'."""
    nums1 = set()
    nums2 = set()
    for t in name1.split():
        n = normalize_number_token(t)
        if n is not None:
            nums1.add(n)
    for t in name2.split():
        n = normalize_number_token(t)
        if n is not None:
            nums2.add(n)
...
SCHOOL_TYPES = {'sd', 'smp', 'sma', 'smk', 'slb', 'mi', 'mts', 'ma', 'tk', 'kb', 'paud', 'sps', 'ra', 'pkbm', 'tpa', 'lpk', 'lkp', 'skb', 'spnf', 'kejar', 'ponpes', 'pesantren'}

def school_type_match(name1, name2):
    """Ensure we don't match SMA with SMK, or SD with SMP."""
    t1 = set(name1.split())
    t2 = set(name2.split())
    type1 = t1.intersection(SCHOOL_TYPES)
    type2 = t2.intersection(SCHOOL_TYPES)
    if type1 and type2 and not type1.intersection(type2):
        return False
    return True

def convert_number_words(name: str) -> str:
    num_map = {
    ...
    for t in tokens:
        if t in num_map:
            new_tokens.append(num_map[t])
        elif t.startswith('0') and len(t) > 1 and t.isdigit():
            new_tokens.append(str(int(t)))
        else:
            new_tokens.append(t)
...
def normalize(name):
    ...
    name = convert_number_words(name)
    
    # Standarisasi tipe sekolah
    for pattern, replacement in SCHOOL_STANDARDIZATION_COMPILED:
...
IGNORE_WORDS = {
    ...
    "dusun", "dsn", "kampung", "kp", "gang", "gg", "lorong", "lr", 
    "blok", "kav", "kavling"
}
...
def compare_tokens(name1, name2):
    tokens1 = name1.split()
    tokens2 = name2.split()
    if not tokens1 or not tokens2: return 0.0, 0.0
    ...
    for t1 in tokens1:
        ...
        for j, t2 in enumerate(tokens2):
            if j in used_t2: continue
            if t1 == t2 or normalize_number_token(t1) == normalize_number_token(t2):
                best_match_idx = j
                break
        if best_match_idx != -1:
            matched_count += 1
            used_t2.add(best_match_idx)
            
    subset_score = matched_count / len(tokens1)
    if subset_score < 1.0: subset_score = 0.0
    coverage = matched_count / len(tokens2)
    return (subset_score, coverage)
...
def process_batch_bm25(rows, bm25, input_matrix, master_lookup, config, dynamic_patterns=None):
    ...
    for i, row in enumerate(rows):
        ...
        nama_norm = normalize(extract_base_name(row['nama_usaha']))
        ...
        candidates = []
        for k in sorted_k:
            m_id, m_kode, m_nama_norm, m_alamat = master_lookup[f_idx[k]]
            sim = r_fuzz.ratio(nama_norm, m_nama_norm)
            candidates.append((sim, m_id, m_kode, m_nama_norm, m_alamat))
            
        candidates.sort(key=lambda x: x[0], reverse=True)
        top_10 = candidates  # Evaluate all BM25 candidates, don't slice
        
        match_list, possible_list = [], []
        for sim, m_id, m_kode, m_nama_norm, m_alamat in top_10:
            
            # 1. Status Conflict Check
            stat1 = 'negeri' if 'negeri' in nama_norm else 'swasta' if 'swasta' in nama_norm else None
            stat2 = 'negeri' if 'negeri' in m_nama_norm else 'swasta' if 'swasta' in m_nama_norm else None
            
            if stat1 and stat2 and stat1 != stat2:
                continue # Force UNMATCH due to status conflict
                
            # 2. Strip Status for text comparison to prevent dilution
            n1_clean = re.sub(r'\b(negeri|swasta)\b', '', nama_norm)
            n2_clean = re.sub(r'\b(negeri|swasta)\b', '', m_nama_norm)
            n1_clean = re.sub(r'\s+', ' ', n1_clean).strip()
            n2_clean = re.sub(r'\s+', ' ', n2_clean).strip()
            
            # Recalculate scores using cleaned names
            sim = r_fuzz.ratio(n1_clean, n2_clean)
            comp = compare_tokens(n1_clean, n2_clean)
            sim_tsr = r_fuzz.token_set_ratio(n1_clean, n2_clean)
            
            if row.get('is_prepended'):
                orig_nama = normalize(extract_base_name(row.get('nama_usaha_original', '')))
                orig_clean = re.sub(r'\b(negeri|swasta)\b', '', orig_nama)
                orig_clean = re.sub(r'\s+', ' ', orig_clean).strip()
                
                sim_orig = r_fuzz.ratio(orig_clean, n2_clean)
                comp_orig = compare_tokens(orig_clean, n2_clean)
                sim_tsr_orig = r_fuzz.token_set_ratio(orig_clean, n2_clean)
                
                sim = max(sim, sim_orig)
                sim_tsr = max(sim_tsr, sim_tsr_orig)
                if (comp_orig[0] + comp_orig[1]) > (comp[0] + comp[1]):
                    comp = comp_orig
            
            valid_numbers = numbers_match(nama_norm, m_nama_norm)
            valid_types = school_type_match(nama_norm, m_nama_norm)
            valid_chars = single_char_match(nama_norm, m_nama_norm)
            
            if not valid_chars:
                continue
            
            res_type = 'UNMATCH'
            if sim >= 95 and comp[1] >= 0.90 and valid_types: res_type = 'MATCH'
            elif sim >= 90 and comp[1] == 1.0 and valid_types: res_type = 'MATCH'
            elif sim_tsr >= 95 and (min(comp) >= 0.66 or sim >= 80) and valid_numbers and valid_types: res_type = 'MATCH'
            elif sim >= 88 and min(comp) >= 0.66 and valid_numbers and valid_types: res_type = 'MATCH'
            elif sim_tsr >= 85 and max(comp) >= 0.50: res_type = 'POSSIBLE'
            elif sim >= 70 and max(comp) >= 0.40: res_type = 'POSSIBLE'
            elif sim >= 80: res_type = 'POSSIBLE'
            
            obj = {"idsbr": m_kode, "id": m_id, "alamat": m_alamat, "type": res_type}
...
def process_kabupaten(kab_id, config):
    ...
    try:
        ...
        m_query = """
            SELECT id, nama, alamat, kode FROM matchapro_business_perusahaan
        ...
        for idx, row in master_df.iterrows():
            n_name = normalize(extract_base_name(row['nama']))
            ...
            master_lookup.append((row['id'], row['kode'], n_name, row['alamat']))
        bm25.fit(corpus)
        ...
        # Load Input
        i_query = f"""
            SELECT id_table, nama_etl as nama_usaha, alamat_etl as alamat, jenjang
            FROM {table_name}
        ...
        for r in rows:
            ...
            jenjang = str(r.get('jenjang') or '').strip().lower()
            if jenjang == 'non formal': jenjang = 'pkbm'
            
            r['nama_usaha_original'] = r['nama_usaha']
            
            nama_norm = normalize(extract_base_name(r['nama_usaha']))
            t = set(nama_norm.split())
            if not t.intersection(SCHOOL_TYPES) and jenjang:
                r['nama_usaha'] = f"{jenjang} {r['nama_usaha']}"
                nama_norm = normalize(extract_base_name(r['nama_usaha']))
                r['is_prepended'] = True
                
            input_corpus.append(nama_norm + " " + " ".join(tokenize_address(alamat_norm)[:5]))
